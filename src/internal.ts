...
export class ErrorCorrection {
  ...
  public static readonly LOW = new ErrorCorrection(0, 1);
  public static readonly MEDIUM = new ErrorCorrection(1, 0);
  public static readonly QUARTILE = new ErrorCorrection(2, 3);
  public static readonly HIGH = new ErrorCorrection(3, 2);
...
export class QrCode {
  ...
  public static encodeSegments(
    ...
    [ErrorCorrection.MEDIUM, ErrorCorrection.QUARTILE, ErrorCorrection.HIGH].forEach(
...
class ReedSolomonGenerator {
  ...
  public getRemainder(data: Array<byte>): Array<byte> {
    ...
    const result: Array<byte> = this.coefficients.map(() => 0);
    data.forEach((b: byte) => {
      const factor: byte = b ^ (result.shift() as int);
      result.push(0);
      for (let i = 0; i < result.length; i++)
        result[i] ^= ReedSolomonGenerator.multiply(
          this.coefficients[i],
          factor
        );
    });
