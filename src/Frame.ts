...
export class QrCode {
  ...
  public static encodeText(text: string, ecl: QrCode_Ecc): QrCode {
    let segs: Array<QrSegment> = QrSegment.makeSegments(text);
  ...
  public static encodeBinary(data: Array<byte>, ecl: QrCode_Ecc): QrCode {
    let seg: QrSegment = QrSegment.makeBytes(data);
  ...
  public static encodeSegments(
    ...
    [QrCode_Ecc.MEDIUM, QrCode_Ecc.QUARTILE, QrCode_Ecc.HIGH].forEach(
      (newEcl: QrCode_Ecc) => {
        // From low to high
        if (
          boostEcl &&
...
<svg xmlns="http://www.w3.org/2000/svg" version="1.1" viewBox="0 0 ${
      this.size + border * 2
    } ${this.size + border * 2}" stroke="none">
...
class ReedSolomonGenerator {
  ...
  public getRemainder(data: Array<byte>): Array<byte> {
    ...
    const result: Array<byte> = this.coefficients.map((_) => 0);
    data.forEach((b: byte) => {
